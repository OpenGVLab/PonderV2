...
from meta_data.scannet200_constants import VALID_CLASS_IDS_20, VALID_CLASS_IDS_200
...
if __name__ == "__main__":
    ...
    labels_pd = pd.read_csv(
        "ponder/datasets/preprocessing/scannet/meta_data/scannetv2-labels.combined.tsv",
        sep="\t",
        header=0,
    )
    ...
    with open(
        "ponder/datasets/preprocessing/scannet/meta_data/scannetv2_train.txt"
    ) as train_file:
        train_scenes = train_file.read().splitlines()
    with open(
        "ponder/datasets/preprocessing/scannet/meta_data/scannetv2_val.txt"
    ) as val_file:
